...
require_once("../../../connections/connection.php"); 
   ...
   no configurado, establecerá la página en 1. */
    if(isset($_GET["pagina"])){
        if($_GET["pagina"]==1){
            header("Location:activarconduct.php");
        }else{
            $pagina=$_GET["pagina"];
        }
    }else{
    ...
    /* Esta es una consulta a la base de datos para obtener todos los usuarios. */
    ...
    $regis=$base->query("SELECT * from usuarios, rol, estado, estado_asig where usuarios.id_rol=4 and usuarios.id_est =2  and usuarios.id_rol=rol.id_rol and usuarios.id_est=estado.id_est and usuarios.id_estvehi=estado_asig.id_estvehi LIMIT $empieza, $regis")->fetchALL(PDO::FETCH_OBJ);
?>
            ...
            <main class="main col">
                <div class="row justify-content-center align-content-center text-center">
                    <div class="col-sm-12" id="title">
                        <h3 class="mb-0 ">Conductores Inactivos</h3>
                    </div>
                </div>
                <br>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="table-responsive" id="contenido">        
                                <table id="tablaRoles" class="table table-striped table-bordered table-condensed" style="width:100%">
                                    ...
                                    <tbody class="text-center">
                                        ...
                                        <tr class="table-light" >
                                            ...
                                            <!-- Crear un enlace a la página ver.php y pasar el id
                                            y rol variables a la página. -->
                                            <td>
                                                <a href="ver2.php?id=<?php echo $usuarios->id_usu?> & rol=<?php echo $usuarios->id_rol?>"><button type="button" title="Ver" class="btn btn-sm btn-primary"><i class="fa-solid fa-magnifying-glass"></i></button></a>
                                            </td>
                                            <!-- Un enlace a un archivo php que activa al usuario. -->
                                            <td>
                                                <a href="activa.php?id=<?php echo $usuarios->id_usu?> & rol=<?php echo $usuarios->id_rol?> & email=<?php echo $usuarios->email?>"><button type="button" title="Activar" class="btn btn-sm btn-blue"><i class="fa-sharp fa-solid fa-circle-check"></i></button></a>
                                            </td>
                                        </tr>
                                        <?php
                                        endforeach;
                                ...
                                <!-- Paginación -->
                                <!-- Comprobando si la página es menor o igual a 1, si lo es, lo hará
                                desactivar el botón. Si no es así, habilitará el botón. -->
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item <?php echo $pagina<=1? 'disabled' : '' ?>">
                                                <a class="page-link" href="activarconduct.php?pagina=<?php echo $pagina-1 ?>">Anterior</a>
                                        </li>
                                        <?php
                                            /* Creando un bucle que creará una lista de páginas. */
                                            for($i=0; $i<$paginas; $i++):?>
                                                <li class="page-item <?php echo $pagina==$i+1? 'active': ''?>">
                                                    <a class="page-link" 
                                                    href="activarconduct.php?pagina=<?php echo $i+1?>">
                                                <?php echo $i+1?></a>
                                                </li>
                                                <?php endfor?>
                                        <li class="page-item <?php  echo $pagina>=$paginas? 'disabled' : '' ?> "><a class="page-link" href="activarconduct.php?pagina=<?php echo $pagina+1 ?>">Siguiente</a></li>
                                    </ul>
                                </nav> 
                            </div>
                        </div>
