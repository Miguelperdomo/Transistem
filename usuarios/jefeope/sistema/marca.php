...
if(isset($_POST['insert'])){
    ...
    $sql="INSERT INTO marcas (id_marca, marca) values (:id, :ma)";
    $resultado=$base->prepare($sql);
...
$regis = 4;
   ...
   /* Obteniendo todos los datos de la tabla marcas. */
    ...
    $regis=$base->query("SELECT * from marcas LIMIT $empieza, $regis")->fetchALL(PDO::FETCH_OBJ);
?>
        ...
        <div class="row">
            ...
            <main class="main col">
                ...
                <div class="container">
                            ...
                            <div class="table-responsive ">        
                                <table id="tablaRoles" class="table table-striped table-bordered table-condensed center3" style="width:50%">
                                    ...
                                    <tbody class="text-center">
                                        <?php
                                            foreach ($regis as $marcas):
                                        ?> 
                                        <tr class="table-light" >
                                            <td><?php echo $marcas->id_marca?></td>
                                            <td><?php echo $marcas->marca?></td>
                                            <td>
                                                <a href="marcas/edit.php?id=<?php echo $marcas->id_marca?> & nom=<?php echo $marcas->marca?>"><button type="button" class="btn btn-sm btn-blue"><i class="fa-solid fa-pen-to-square"></i></button></a>
                                            </td>
                                            <td>
                                                <a href="marcas/delete.php?id=<?php echo $marcas->id_marca?> & nom=<?php echo $marcas->marca?>"><button type="button" class="btn btn-sm btn-danger"> <i class="fa-solid fa-trash-can"></i></button></a>
                                            </td>
                                        </tr>
                                        <?php
                                        endforeach;
                    ...
                    <!--  Creación de un sistema de paginación para el sitio web.. -->
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?php echo $pagina<=1? 'disabled' : '' ?>">
                                    <a class="page-link" href="marca.php?pagina=<?php echo $pagina-1 ?>">Anterior</a>
                            </li>
                            <?php
                                /* Creando un sistema de paginación para la página. */
                                for($i=0; $i<$paginas; $i++):?>
                                    <li class="page-item <?php echo $pagina==$i+1? 'active': ''?>">
                                        <a class="page-link" 
                                        href="marca.php?pagina=<?php echo $i+1?>">
                                    <?php echo $i+1?></a>
                                    </li>
                                    <?php endfor?>
                            <li class="page-item <?php  echo $pagina>=$paginas? 'disabled' : '' ?> "><a class="page-link" href="marca.php?pagina=<?php echo $pagina+1 ?>">Siguiente</a></li>
                        </ul>
                    </nav> 
            </main>
