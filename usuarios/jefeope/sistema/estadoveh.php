...
if(isset($_POST['insert'])){
    ...
    $sql="INSERT INTO estado_asig (id_estvehi, estvehi) values (:id, :est)";
    $resultado=$base->prepare($sql);
    $resultado->execute(array(":id"=>$id, ":est"=>$mar));

    header("Location:estadoveh.php");
}
...
no configurado, establecerá la página en 1. */
   ...
   /* Obteniendo todos los datos de la tabla estado_asig. */
    $sql= 'SELECT * FROM estado_asig';
    ...
    $regis=$base->query("SELECT * from estado_asig LIMIT $empieza, $regis")->fetchALL(PDO::FETCH_OBJ);
?>
            ...
            <main class="main col">
                <div class="row justify-content-center align-content-center text-center">
                    <div class="col-sm-9" id="title">
                        <h3 class="mb-0 ">Estados Vehiculos</h3>
                    </div>
                    <br>
                    <!-- Modal Insertar -->
                    <div class="container">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form method="post" autocomplete="off">
                                            ...
                                            <div class="col-auto">
                                                <div class="input-group col ">
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text">Estado Vehiculo</div>
                                                    </div>
                                                    <input type="text" required class="form-control" name="mar" >
                                                </div>
                                            </div>
                                            <br>                                
                ...
                <div class="container">
                            ...
                            <div class="table-responsive ">        
                                <table id="tablaRoles" class="table table-striped table-bordered table-condensed center3" style="width:50%">
                                    <thead class="text-center ">
                                        <tr>
                                            <!--Creando una tabla con las siguientes columnas: Id, Estado,
                                            Acciones. -->
                                             <th>Id</th>
                                             <th>Estado</th>
                                    ...
                                    <tbody class="text-center">
                                        <?php
                                            /* Creating a foreach loop that will loop through the
                                             array and assign each value to the 
                                            variable. */
                                            foreach ($regis as $estado):
                                        ?> 
                                        <tr class="table-light" >

                                            <!-- El código anterior es un código PHP que se utiliza para mostrar el
                                            datos de la base de datos. -->
                                            <td><?php echo $estado->id_estvehi?></td>
                                            <td><?php echo $estado->estvehi?></td>
                                            <td>
                                                <a href="estadoveh/edit.php?id=<?php echo $estado->id_estvehi?> & nom=<?php echo $estado->estvehi?>"><button type="button" class="btn btn-sm btn-blue"><i class="fa-solid fa-pen-to-square"></i></button></a>
                                            </td>
                                            <td>
                                                <a href="estadoveh/delete.php?id=<?php echo $estado->id_estvehi?> & nom=<?php echo $estado->estvehi?>"><button type="button" class="btn btn-sm btn-danger"> <i class="fa-solid fa-trash-can"></i></button></a>
                                            </td>
                                        </tr>
                                        <?php
                                        endforeach;
                    ...
                    <!-- Paginación -->
                    <!-- El código anterior está creando un sistema de paginación para la tabla. -->
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?php echo $pagina<=1? 'disabled' : '' ?>">
                                    <a class="page-link" href="estadoveh.php?pagina=<?php echo $pagina-1 ?>">Anterior</a>
                            </li>
                            <?php
                                for($i=0; $i<$paginas; $i++):?>
                                    <li class="page-item <?php echo $pagina==$i+1? 'active': ''?>">
                                        <a class="page-link" 
                                        href="estadoveh.php?pagina=<?php echo $i+1?>">
                                    <?php echo $i+1?></a>
                                    </li>
                                    <?php endfor?>
                            <li class="page-item <?php  echo $pagina>=$paginas? 'disabled' : '' ?> "><a class="page-link" href="estadoveh.php?pagina=<?php echo $pagina+1 ?>">Siguiente</a></li>
                        </ul>
                    </nav> 
            </main>
