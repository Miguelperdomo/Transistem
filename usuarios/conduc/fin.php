...
/* Una consulta a la base de datos trayendo las tablas rol y usuarios y concatenando el id_rol con usuarios y el id_usu con la variable Global session  */
 ...
 /* Una consulta a la base de datos que trae las tablas users, detalle_vehi where detalle_vehi.id_usu =
 usuarios.id_usu y detalle_vehi.id_usu = $ doc; */
 $sql= "SELECT * FROM usuarios, detalle_vehi where detalle_vehi.id_usu=usuarios.id_usu and detalle_vehi.id_usu= $doc"; 
 $resultado=$base->prepare($sql);
 $resultado->execute(array());
...
/* Esta es una sentencia condicional que comprueba si la `fecha_ini_real` está vacía o no. Si está vacía,
imprimirá un mensaje que dice "No hay ruta iniciada." y redirige a `verprogra.php`. */
if(empty($orden['fecha_ini_real'])){
    echo '<script>alert("No hay iniciado ruta.");</script>';//Imprime un mensaje de que el viaje a Finalizado
    echo '<script>window.location= "verprogra.php"</script>';

}
/* Esta es una sentencia condicional que comprueba si la `fecha_fin_real` está vacía o no. Si está vacía
imprimirá un mensaje que dice "Ya fianlizó la ruta." y redirige a `verprogra.php`. */ 
elseif($orden['fecha_fin_real']){
    echo '<script>alert("Ya fianlizó la ruta.");</script>';//Imprime un mensaje de que el viaje a Finalizado
    echo '<script>window.location= "verprogra.php"</script>';
}
else{
    ...
    if(isset($_POST['programar'])){
        ...
        // $id=$orden['id_auto']; 
        $idsoli=$orden['id_soli'];
        $fecha= date("Y-m-d H:i:s"); 
        $vehiculo=$vehi['placa'];
        $estVehi=6;

        $sql="UPDATE  orden_ser SET fecha_fin_real=:fec WHERE id_auto=:id";
        ...
        echo '<script>alert("Ha finalizado el viaje.");</script>';//Imprime un mensaje de que el viaje a Finalizado
         ...
         <main class="main col">
             <div class="row justify-content-center align-content-center text-center">
                 <div class="col-sm-9" id="title">
                     <h3 class="mb-0 ">Finalizar Viaje</h3>
                 </div>
                 <br>
                         ...
                         <div class="modal-body">
                             <form method="post">
                                <div class="col-auto">
                                 ...
                                 <br> 
                                   <!-- Aqui esta el boton finalizar -->                             
                                 <div class="modal-footer">
                                     <a href="verprogra.php"><button type="button" class="btn btn-secondary">Cancelar</button></a>
                                     <input  type="submit" class="btn btn-blue" name="programar" value="Finalizar" >
                                 </div>
                             </form>
                         </div>
             </div>
