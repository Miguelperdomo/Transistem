...
if(isset($_POST['insert'])){
    ...
    $sql="INSERT INTO tipo_cliente (id_tip_clien, tip_clien) values (:id, :tip)";
    $resultado=$base->prepare($sql);
    $resultado->execute(array(":id"=>$id, ":tip"=>$tip));

    header("Location:tipcli.php");
}
...
$regis = 4;
    ...
    $regis=$base->query("SELECT * from tipo_cliente LIMIT $empieza, $regis")->fetchALL(PDO::FETCH_OBJ);
?>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Añadir Tipo persona</h5>
                            ...
                            <div class="table-responsive ">    
                                ...
                                <table id="tablaRoles" class="table table-striped table-bordered table-condensed center3" style="width:50%">
                                    ...
                                    <tbody class="text-center">
                                        <?php
                                            foreach ($regis as $tipo_cliente):
                                        ?> 
                                        <tr class="table-light" >
                                            <td><?php echo $tipo_cliente->id_tip_clien?></td>
                                            <td><?php echo $tipo_cliente->tip_clien?></td>
                                            <td>
                                                <a href="tipclien/edit.php?id=<?php echo $tipo_cliente->id_tip_clien?> & nom=<?php echo $tipo_cliente->tip_clien?>"><button type="button" class="btn btn-sm btn-blue"><i class="fa-solid fa-pen-to-square"></i></button></a>
                                            </td>
                                            <td>
                                                <a href="tipclien/delete.php?id=<?php echo $tipo_cliente->id_tip_clien?> & nom=<?php echo $tipo_cliente->tip_clien?>"><button type="button" class="btn btn-sm btn-danger"> <i class="fa-solid fa-trash-can"></i></button></a>
                                            </td>
                                        </tr>
                                        <?php
                                        endforeach;
                    ...
                    <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item <?php echo $pagina<=1? 'disabled' : '' ?>">
                                                <a class="page-link" href="tipcli.php?pagina=<?php echo $pagina-1 ?>">Anterior</a>
                                        </li>
                                        <?php
                                            for($i=0; $i<$paginas; $i++):?>
                                                <li class="page-item <?php echo $pagina==$i+1? 'active': ''?>">
                                                    <a class="page-link" 
                                                    href="tipcli.php?pagina=<?php echo $i+1?>">
                                                <?php echo $i+1?></a>
                                                </li>
                                                <?php endfor?>
                                        <li class="page-item <?php  echo $pagina>=$paginas? 'disabled' : '' ?> "><a class="page-link" href="tipcli.php?pagina=<?php echo $pagina+1 ?>">Siguiente</a></li>
                                    </ul>
                                </nav> 
            </main>
