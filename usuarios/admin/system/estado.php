...
if(isset($_POST['insert'])){
    ...
    $sql="INSERT INTO estado (id_est, tip_est) values (:id, :es)";
    $resultado=$base->prepare($sql);
    $resultado->execute(array(":id"=>$id, ":es"=>$est));
...
$regis = 4;
   ...
   /* Obtención de datos de la base de datos. */
    $regis=$base->query("SELECT * from estado LIMIT $empieza, $regis")->fetchALL(PDO::FETCH_OBJ);
?>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Añadir Estados</h5>
                                    ...
                                    <!-- /* formulario con una entrada de texto y un botón de envío.*/ -->
                                        <form method="post" autocomplete="off">
                                            ...
                                            <div class="col-auto">
                                                <div class="input-group col ">
                                                    <div class="input-group-prepend">
                                                        <div class="input-group-text">Estados</div>
                                                    </div>
                                                    <input type="text" required class="form-control" name="es" >
                                                </div>
                                            </div>
                                            <br>                                
                                ...
                                <table id="tablaRoles" class="table table-striped table-bordered table-condensed center3" style="width:50%">
                                    ...
                                    <tbody class="text-center">
                                        ...
                                        <tr class="table-light" >
                                            <td><?php echo $estado->id_est?></td>
                                            <td><?php echo $estado->tip_est?></td>
                                            <td>
                                                <a href="estados/edit.php?id=<?php echo $estado->id_est?>"><button type="button" class="btn btn-sm btn-blue"><i class="fa-solid fa-pen-to-square"></i></button></a>
                                            </td>
                                            <td>
                                                <a href="estados/delete.php?id=<?php echo $estado->id_est?>"><button type="button" class="btn btn-sm btn-danger"> <i class="fa-solid fa-trash-can"></i></button></a>
                                            </td>
                                        </tr>
                                        <?php
                                        endforeach;
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                        <li class="page-item <?php echo $pagina<=1? 'disabled' : '' ?>">
                                <a class="page-link" href="estado.php?pagina=<?php echo $pagina-1 ?>">Anterior</a>
                        </li>
                        <?php
                            for($i=0; $i<$paginas; $i++):?>
                                <li class="page-item <?php echo $pagina==$i+1? 'active': ''?>">
                                    <a class="page-link" 
                                    href="estado.php?pagina=<?php echo $i+1?>">
                                <?php echo $i+1?></a>
                                </li>
                                <?php endfor?>
                        <li class="page-item <?php  echo $pagina>=$paginas? 'disabled' : '' ?> "><a class="page-link" href="estado.php?pagina=<?php echo $pagina+1 ?>">Siguiente</a></li>
                    </ul>
                </nav> 
            </main>
